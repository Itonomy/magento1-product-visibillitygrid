...
class Itonomy_ProductVisibilityGrid_Model_Product_Collection extends Mage_Core_Model_Resource_Db_Collection_Abstract
{

    protected $_idFieldName = 'entity_id';
    protected $_storeId = Mage_Core_Model_App::ADMIN_STORE_ID;
    protected $_attributes = array('name', 'status');
    ...
    {
        $this->_resourceModel = 'catalog/product';
        $this->_mainTable = $this->getTable('catalog/product');
        parent::__construct();
    }
     ...
     *
     * @return Itonomy_ProductVisibilityGrid_Model_Product_Collection
     */
    public function prepareSelect()
    {
        ...
        if ($this->_storeId != Mage_Core_Model_App::ADMIN_STORE_ID) {
            ...
            $flatTable = $this->getTable('catalog/product_flat') . '_' . $this->_storeId;
            ...
            if ($this->getConnection()->isTableExists($flatTable)) {
                ...
                $select->joinLeft(
                    ...
                    array(
                        ...
                        'visibility' => new Zend_Db_Expr('COALESCE(flat_table.visibility, ' . Mage_Catalog_Model_Product_Visibility::VISIBILITY_NOT_VISIBLE . ')')
                    )
                );
            } else {
                ...
                $select->columns(array(
                    ...
                    'visibility' => new Zend_Db_Expr(Mage_Catalog_Model_Product_Visibility::VISIBILITY_NOT_VISIBLE)
                ));
            }
            ...
            $select->joinLeft(
                ...
                . ' AND category_product.store_id = \'' . $this->_storeId . '\''
                . ' AND category_product.visibility > \'' . Mage_Catalog_Model_Product_Visibility::VISIBILITY_NOT_VISIBLE . '\'',
                array('in_category' => new Zend_Db_Expr('category_product.product_id IS NOT NULL'))
            );

            // Join with stock status table.
            $select->joinLeft(
                array('stock_status' => $this->getTable('cataloginventory/stock_status_indexer_idx')),
                'stock_status.product_id = main_table.entity_id'
                . ' AND stock_status.stock_id = \'1\''
                . ' AND stock_status.website_id = \'' . $store->getWebsiteId() . '\'',
            ...
            $select->joinLeft(
                ...
                . ' AND price_index.customer_group_id = \'' . Mage_Customer_Model_Group::NOT_LOGGED_IN_ID . '\''
            ...
            foreach (Mage::app()->getStores() as $store) {
                $storeId = $store->getId();
                $flatTable = $this->getTable('catalog/product_flat') . '_' . $storeId;
    ...
    protected function _addAttribute($name)
    {
        ...
        $select->joinLeft(
            array($table => $this->getTable('catalog/product') . '_' . $attributeType),
            $table . '.entity_id = main_table.entity_id'
            . ' AND ' . $table . '.attribute_id = \'' . $attributeId . '\''
            . ' AND ' . $table . '.store_id = \'' . Mage_Core_Model_App::ADMIN_STORE_ID . '\'',
            array()
        );
        ...
        if ($this->_storeId != Mage_Core_Model_App::ADMIN_STORE_ID) {
            ...
            $select->joinLeft(
                array($tableStore => $this->getTable('catalog/product') . '_' . $attributeType),
                $tableStore . '.entity_id = main_table.entity_id'
                . ' AND ' . $tableStore . '.attribute_id = \'' . $attributeId . '\''
                . ' AND ' . $tableStore . '.store_id = \'' . $this->_storeId . '\'',
                array()
            );
    ...
    protected function _getConditionSql($fieldName, $condition)
    {
        ...
        if ($this->_storeId != Mage_Core_Model_App::ADMIN_STORE_ID && in_array($fieldName, $this->_attributes)) {
            return new Zend_Db_Expr(
                '(product_' . $fieldName . '_store.value_id IS NOT NULL AND ' . parent::_getConditionSql('product_' . $fieldName . '_store.value',
                    $condition) . ')'
                . ' OR ' . '(product_' . $fieldName . '_store.value_id IS NULL AND ' . parent::_getConditionSql('product_' . $fieldName . '.value',
                    $condition) . ')'
            );
        } else {
